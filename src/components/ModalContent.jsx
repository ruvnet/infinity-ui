...
import { Brain, Activity, Zap, Cpu, Atom, GitBranch, Shuffle, Compass, Clock, AlertTriangle, ZapIcon, RotateCw, Radio, Inbox, Send, Plus } from 'lucide-react';
import { DashboardItem, ProgressBar, PredictionDisplay, FlashingText } from './ModalComponents';

export const Communications = () => {
  const [messages, setMessages] = useState([
    { id: 1, from: 'Zorg-9', subject: 'Intergalactic Treaty', read: false },
    { id: 2, from: 'Quantum AI', subject: 'Simulation Results', read: true },
    { id: 3, from: 'Time Council', subject: 'Paradox Alert', read: false },
  ]);

  const [selectedMessage, setSelectedMessage] = useState(null);
  const [showNewMessage, setShowNewMessage] = useState(false);

  const recipients = ['Zorg-9', 'Quantum AI', 'Time Council', 'Galactic Federation', 'Nebula Outpost'];

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <DashboardItem icon={Radio} label="Subspace Link" value="Active" />
        <DashboardItem icon={Inbox} label="Unread Messages" value={messages.filter(m => !m.read).length} />
        <DashboardItem icon={Send} label="Outgoing Queue" value="2" />
        <DashboardItem icon={Zap} label="Signal Strength" value="98%" />
      </div>
      <ProgressBar label="Encryption Level" value={92} max={100} />
      <div className="mt-4 bg-[#b73616]/30 p-3 rounded-lg">
        <div className="flex justify-between items-center mb-2">
          <div className="text-[#ffd0a8] text-sm font-semibold">Inbox</div>
        ...
        {showNewMessage ? (
          <div className="space-y-2">
            ...
            <input type="text" placeholder="Subject" className="w-full bg-[#b73616] text-[#ffd0a8] border border-[#ffd0a8] rounded p-1" />
            <textarea placeholder="Message" className="w-full bg-[#b73616] text-[#ffd0a8] border border-[#ffd0a8] rounded p-1" rows="3"></textarea>
            <button className="bg-[#ffd0a8] text-[#b73616] px-2 py-1 rounded hover:bg-[#ffd0a8]/80 transition-colors">Send</button>
          </div>
        ) : (
          <div className="space-y-2">
      ...
      {selectedMessage && (
        <div className="mt-4 bg-[#b73616]/30 p-3 rounded-lg">
          ...
          <div className="text-[#ffd0a8] text-sm">
            Message content for {selectedMessage.subject} from {selectedMessage.from}...
          </div>
        </div>
      )}
    </div>
